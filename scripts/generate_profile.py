    ...
    def get(self, value_name):
        for value in self.values:
            if value.name == value_name:
                return value
        raise AssertionError(f"Invalid value name {value_name} in type {self.name}")

    def __str__(self):
        s = 'FieldType(%s\n' % render_comment(self.comment)
        s += "    name='%s',\n" % (self.name)
        s += f"    base_type=BASE_TYPES[{BASE_TYPES[self.base_type]}],  # {self.base_type}\n"
...
class TypeValueInfo(namedtuple('TypeValueInfo', ('name', 'value', 'comment'))):
    def __str__(self):
        return f"{self.value}: '{self.name}',{render_comment(self.comment)}"
...
class MessageList(namedtuple('MessageList', ('messages'))):
    def __str__(self):
        ...
        for message in sorted(
            ...
            s += f"    {message.num}: {indent(message)},\n"
        s += '}'
        return s
    ...
    def get_field_by_name(self, mesg_name, field_name):
        ...
        raise ValueError(f'field "{field_name}" not found in message "{mesg_name}"')
...
class MessageInfo(namedtuple('MessageInfo', ('name', 'num', 'group_name', 'fields', 'comment'))):
    def get(self, field_name):
        for field in self.fields:
            if field.name == field_name:
                return field
        raise AssertionError(f"Invalid field name {field_name} in message {self.name}")
...
class FieldInfo(namedtuple('FieldInfo', ('name', 'type', 'num', 'scale', 'offset', 'units', 'components', 'subfields', 'comment'))):
    def __str__(self):
        if self.num == FIELD_NUM_TIMESTAMP:
            ...
            return f'FIELD_TYPE_TIMESTAMP,{render_comment(self.comment)}'
        s = "Field(%s\n" % render_comment(self.comment)
        s += "    name='%s',\n" % self.name
...
def fix_scale(data):
    return None if data == 1 else data
...
def parse_csv_fields(data, num_expected):
    ...
    return ret * num_expected if len(ret) == 1 and num_expected else ret
...
def main(input_xls_or_zip, output_py_path=None):
    ...
    for mesg_name in MESSAGE_NUM_DECLARATIONS:
        ...
        mesg_num_declarations.append(
            f"MESG_NUM_{scrub_symbol_name(mesg_name).upper()} = {str(mesg_info.num) if mesg_info else 'None'}"
        )


    field_num_declarations = [f'FIELD_NUM_TIMESTAMP = {str(FIELD_NUM_TIMESTAMP)}']
    for field_fqn in FIELD_NUM_DECLARATIONS:
        ...
        field_decl = f'FIELD_NUM_{scrub_symbol_name(mesg_name).upper()}_{scrub_symbol_name(field_name).upper()} = {str(field_info.num)}'
    ...
    output = (
        '\n'.join(
            [
                ...
                header(
                    f"EXPORTED PROFILE FROM {f'SDK VERSION {profile_version}' if profile_version else 'SPREADSHEET'} ON {datetime.datetime.now().strftime('%Y-%m-%d')}"
                ),
                header(
                    'PARSED %d TYPES (%d VALUES), %d MESSAGES (%d FIELDS)'
                    % (
                        len(type_list.types),
                        sum(len(ti.values) for ti in type_list.types),
    ...
    if output_py_path:
        ...
        print(
            f"Profile version {profile_version or '<unknown>'} written to {output_py_path}"
        )
